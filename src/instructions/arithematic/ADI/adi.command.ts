...
export const ADI = (): { [key: string]: InstructionType } => {
  const MakeADIInstruction = (opcode: string): InstructionType => {
    ...
    return {
      ...
      execute: (state, dispatch) => {
        ...
        const { result, flags } = addition(accumulatorValue, value);
